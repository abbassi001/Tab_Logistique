...
/**
 * Initialize transport types chart
 */
function initializeTransportChart() {
    ...
    // Get data from the data attributes
    const chartElement = transportCtx.canvas;
    const labels = JSON.parse(chartElement.dataset.labels || '["Aérien", "Maritime", "Ferroviaire", "Routier", "Autre"]');
    const data = JSON.parse(chartElement.dataset.values || '[15, 30, 10, 40, 5]');
    const colors = JSON.parse(chartElement.dataset.colors || '["#4e73df", "#1cc88a", "#36b9cc", "#f6c23e", "#e74a3b"]');
...
function setupExportButtons() {
    // Ajouter un sélecteur plus spécifique pour ne cibler que les boutons d'export
    const exportButtons = document.querySelectorAll('.dropdown-item:not([href="/logout"])');
    
    exportButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            e.preventDefault();
            
            // Vérifier que ce n'est pas un lien de déconnexion
